...
#include "console_v1.5.5.h" /* cabe‹¨«alho de todas as novas fun‹¨«‹¨«es definidas neste arquivo */
...
EVENTO Evento(void)
{
	...
	switch(irInBuf.EventType)
	{
		   case KEY_EVENT: /* keyboard input  */
				...
				Dados_evento.teclado.key_code = irInBuf.Event.KeyEvent.wVirtualKeyCode; /* armazenar o c‹¨«digo da tecla
				pressionada */
				Dados_evento.teclado.scan_code = irInBuf.Event.KeyEvent.wVirtualScanCode;
				
				GetKeyboardState(lpKeyState);
				ToAscii(Dados_evento.teclado.key_code, Dados_evento.teclado.scan_code, lpKeyState, (LPWORD)&Dados_evento.teclado.ascii_code, 0);	
				
                Dados_evento.teclado.repeat_count = irInBuf.Event.KeyEvent.wRepeatCount; 
...
void IdentificarEvento(EVENTO e, int nivel_detalhe)
{
     switch(e.tipo_evento) /* identificar o tipo do evento recebido como par‹¨«metro */
	 {
			...
			case KEY_EVENT:
			     printf("\nEvento do teclado");
				 printf("\n-------------------");
				 printf("\n-> Key code da tecla: %d", e.teclado.key_code);
				 printf("\n-> Scan code  da tecla: %d", e.teclado.scan_code);
				 printf("\n-> Codigo ascii da tecla: %d", e.teclado.ascii_code);
